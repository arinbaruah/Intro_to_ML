...
#### 1) When the threshold value for classification is 0.3
...
The sensitivity for `model 1` when the threshold value for positive Bilby classification is 0.3 and above is __`r sens(model_new_1a,y,pred1) %>% pull(.estimate) %>% round(3)`__. The balanced accuracy for the same model which is based on the true positive and the true negative rates are  __`r spec(model_new_1a,y,pred1) %>% pull(.estimate) %>% round(3)`__.
