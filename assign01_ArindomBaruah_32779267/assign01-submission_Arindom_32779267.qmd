...
@tbl-summary provides a succinct summary of the variance of each principal component and the proportion of variance explained by that particular principal componenet (PC).
...
autoplot(cricket_pca, loadings = TRUE, 
...
```{r}
cricket_pca_var <- tibble(n=1:length(cricket_pca$sdev), evl=cricket_pca$sdev^2)
ggplot(cricket_pca_var, aes(x=n, y=evl)) + geom_line() + geom_point() +
